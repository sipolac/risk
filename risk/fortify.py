...
def fortify(arg_list: List[Dict],
    ...
    arg_list_old = deepcopy(arg_list)  # so we don't modify original
    arg_list = deepcopy(arg_list)

    @utils.memoize  # can't use lru_cache b/c function args are not hashable
    def calc_desired_p(args, arg_idx, terr_idx, p_list):
    ...
    for _ in range(d_troops):
        ...
        for arg_idx, args in enumerate(arg_list):
            for terr_idx in range(len(args['d'])):
                p = calc_desired_p(args, arg_idx, terr_idx, p_list)
                if p < best_p:
                    best_p = p
